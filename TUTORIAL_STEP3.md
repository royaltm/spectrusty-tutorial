...
After finishing the previous steps, you can now enjoy the whole magnificence of the Spectrum's BASIC, we can draw CIRCLEs, change the BORDER color, or even BEEP some tones. That is if we have enough patience to write everything ourselves... without the ability to ever record it. A ZX Spectrum BASIC mandala. Perfect!

What if, theoretically speaking, we would like to have such ability: to SAVE programs... and to LOAD them back. I'm thinking magnetic tapes here, [casettes], yeah those things. Are you still with me? Great!

As your new Spectrum lives in a digital world only, we have to forget about those reel-to-reels. The magnetic fields generated by the recorder's head is now replaced by the digital [TAP] format.

We'd have to feed the Spectrum with some `EAR IN` state changes (a.k.a pulses) somehow. And to read `MIC OUT` state changes also. And those pulses decode as [TAP] bytes and those [TAP] bytes encode as pulses. So much to do... so little time...

But wait. The part of SPECTRUSTY is also ... [spectrusty-formats] crate! Not only we can parse and construct some popular [format] files, but we can also read and convert TAPs to pulses and convert pulses to bytes while writing to TAPs.

First, we'll begin with some more imports...

```rust
use core::convert::TryFrom;
use core::fmt::Write;
use std::fs::{File, OpenOptions};
// we will log some info to help us track actions
...
Next we need to update your emulator and add the TAPE casette recorder to the `ZxSpectrum` struct... emulated, of course. You can also add a boolean if you want to optionally hear the TAPE sound being played or recorded.
...
For this purpose we'll use the [Tape] organizer with the [fs::File] as its data provider. We could also use [`io::Cursor<Vec<u8>>`] or anything that implements [io::Read] + [io::Write] + [io::Seek] as the [Tape] backend.

Because a new `state` property was added to the main struct, we certainly would want to preserve its content when we change the model, wouldn't we?

Here is a refactored version of `ZxSpectrumModel` implementation:
...
The cloning part was delegated to the [From] implemementation, which may look like this:
...
Oh and you may ask, what is this `nmi_request` thing? I'll address this in a moment. But for now let's get back on the TAPE track.

For the sake of simplicity we'll get the TAP file name from the first command argument that your program is started with.
...
But you may ask why? The answer won't be short, if you honestly don't care you may skip to the next paragraph. This method is now refactored in a way that the first part requires, that the previous frame's MIC data is still available. Triggering NMI takes a few CPU cycles. If we were to do it before we call this method (as was implemented before), we'll be losing this data because the `trigger_nmi` will wrap the current frame and clear all temporary buffers. This happens not because triggering NMI is somehow special, but because we are at the very end of the previous frame when the `trigget_nmi` is being invoked. All [ControlUnit] methods that forward the clock counter ensures that the counter doesn't exceed some frame duration limit and calls [ControlUnit::ensure_next_frame] to conditionally prepare for the next frame. If we place the [ControlUnit::nmi] call just before the code is being executed it won't influence any side effects this way. Another funny thing is - and this is not strictly emulation related but is also an issue with the real Z80 CPU - that the NMI will not be triggered if the executed instruction was one of the `0xDD`, `0xFD` prefixes or the `EI` instruction. If you would fill the whole memory with one of these, you'll never be able to trigger the Non-Maskable Interrupt. This is reflected in the return value from the [Cpu::nmi]. Triggering NMI may actually fail. That is why only when it returns `true` the `nmi_request` is being cleared.
...
We would hear the tape sound when we `LOAD` or `SAVE` if `audible_tape` is `true`, otherwise only beeper sound can be heard.

The only thing missing still is the ability to allow the user to somehow control the TAPE. It could be implemented with something similar to the following in the run loop:
...
When `get_user_input_request` is a hypothetical UI event handler with the signature: ```() -> Option<InputRequest>```, the `display_info` will help the user to have a clear picture of what is happening with the TAP file.

The `ZxSpectrum`'s method `update_on_user_request` might look somewhat like this:
...
Let's see the `info` method then, it should be self explanatory:
...
But these games can load for so many minutes. Wow, I remember waiting patiently, fingers crossed, fearing the dread of the `R Tape Loading Error`. Today we don't have such a luxury, time flies differently, so it seems.
...
For the new capabilities, at first you need to add some new properties to your `EmulatorState`:
...
The property `paused` will indicate if the emulation is paused or is it running and `turbo` will indicate if the TURBO mode is on or off. Another new property `flash_tape` if `true` will enable our new FLASH TAPE LOAD and SAVE feature. I'll go into detail about it a little later in this chapter. For now, let's focus on the TURBO mode and the ability to PAUSE your emulator.

In order for the TURBO mode to work, you need to create a new method for `ZxSpectrum`:
...
We also need to slightly change the `run_frame` method's signature and its implementation. Additionally to the flag, it has returned previously, it'll now return the number of T-states that our emulated CPU can grind while the code is being executed. This is not really needed in order to implement the TURBO mode, but it can be used to benchmark the emulator's performance. You might measure the wall time of the execution of this function and divide the `sum` value returned from it by the wall time duration to estimate on how many T-states per some time unit your emulated Spectrum can run.
...
So the next logical step would be to apply some changes to the main emulator loop to actually handle the TURBO mode and the ability to PAUSE our Spectrum.
...
Let's not forget about TURBO and PAUSE features in the input handler:
...
When the TURBO mode is ON, we will call `run_frames_accelerated` instead of `run_frame` and no audio will be rendered. While the emulator is `PAUSED` we'll run a simple inner loop, which just handles two conditions to either exit the main program loop or to exit the paused state.
...
When the feature is ON, whenever the user requests data from the TAPE by e.g. typing `LOAD ""` command, the TAPE will automatically play and at the same time, the TURBO mode will be turned on automatically.
...
The conditions, in order to turn OFF the TURBO mode, would be either:
...
The good news is, for our FLASH TAPE implementation, we only need to make some changes in `run_frame`. However, I think it's time to split this method into smaller ones because it begins to become unreadable. Not only by human programmers. The Rust optimizer doesn't really like large functions.
...
You may also check out [this implementation] which is a more sophisticated version of this method, that can also detect if the ROM loading routine is being called and loads data instantly.

The SAVE detection is simpler but it solely depends on the ability of the TAP writer to decode pulses.

With the final touch, you may update the `info` method to show to the user status of the new awesome features.
...
The [example][src/bin/step3.rs] program using [minifb] and [cpal], covering the scope of this tutorial can be run with:
...
[casettes]: https://en.wikipedia.org/wiki/Cassette_tape
[spectrusty-formats]:
[Tape]:
[fs::File]:
[`io::Cursor<Vec<u8>>`]:
