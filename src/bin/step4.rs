...
*/
//! This is the example implementation of STEP 4 of the SPECTRUSTY tutorial using `minifb` framebuffer
...
#[derive(Default)]
struct ZxSpectrum<C: Cpu, M: ZxMemory, D: BusDevice=TerminatorDevice> {
    cpu: C,
    ula: UlaPAL<M, D>,
    nmi_request: bool,
    state: EmulatorState
...
/*
use spectrusty::bus::joystick::KempstonJoystick;
    ...
    where D: BusDevice<Timestamp=VFrameTs<UlaVideoFrame>> + Default
{
    fn into_cpu_device_and_state(self) -> (C, D, EmulatorState) {
        match self {
            ZxSpectrumModel::Spectrum16(spec16) => (spec16.cpu, spec16.ula.into_bus_device(), spec16.state),
            ZxSpectrumModel::Spectrum48(spec48) => (spec48.cpu, spec48.ula.into_bus_device(), spec48.state),
        }        
    }
    ...
    fn change_model(self, request: ModelReq) -> Self {
        ...
        match request {
            ModelReq::Spectrum16 => Spectrum16(ZxSpectrum16k::<C, D>::from(self)),
            ModelReq::Spectrum48 => Spectrum48(ZxSpectrum48k::<C, D>::from(self))
        }
    }
}
...
fn show_help() -> Result<()> {
    eprintln!("{}: [-16|48] [-b BORDER] [-j JOYSTICK] [TAPFILE]",
            std::env::args().next().as_deref().unwrap_or("step4"));
    Ok(())
}
